...
- (void)showToView{
    ...
    if (self.currentIndex < self.photos.count && [self.photos[self.currentIndex] isKindOfClass:[NSString class]]){//可能是本地也可能是网络Url
        
//        toImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:self.photos[self.currentIndex]]];
    }else if (self.currentIndex < self.photos.count && ![self.photos[self.currentIndex] isKindOfClass:[UIImage class]]) {
